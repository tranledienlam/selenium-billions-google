...
class Node:
    ...
    def _send_screenshot_to_telegram(self, message: str):
        ...
        if response.status_code == 200:
            self.log(f'Hình ảnh được gửi đến bot tele')
        else:
            self.log(
                f'Không thể gửi "Hình ảnh" lên Telegram. Mã lỗi: {response.status_code}. Lưu về local'
            )
            ...
            if choice in ('1', '2', '3'):
                ...
                if not auto:
                    ...
                    for idx, profile in enumerate(profile_list, start=1):
                        print(f"{idx}. {profile['profile_name'] if choice in ('1', '2') else profile}{' [✓]' if choice in ('1', '2') and profile.get('profile_name') in user_data_profiles else ''}")
