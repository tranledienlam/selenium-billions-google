...
📌 **Trang dự án**: [billions.network](https://signup.billions.network/?rc=WGIG2OST)
...
<p align="center">
    <a href="https://youtu.be/suZ2Rfn3j4Q">
        <img src="https://img.youtube.com/vi/suZ2Rfn3j4Q/maxresdefault.jpg" alt="Xem video demo">
    </a>
</p>
...
- **Check-in**: Tự động connect tài khoản gg và check-in hằng ngày.

---

## 🔧 Yêu cầu ban đầu

- Dự án có tích hợp reCaptcha, việc đăng nhập tự động có thể gặp phải captcha. Các phương án xử lý:
  1. Chạy lại chương trình vài lần
  2. Giảm số lượng profile chạy đồng thời
  3. Sử dụng proxy để phân tán IP
  4. Hoặc sử dụng chế độ "1. Set up" để đăng nhập thủ công
  Sau khi đăng nhập thành công, trình duyệt sẽ lưu trạng thái đăng nhập cho những lần sử dụng sau.
...
| `google.py`                    | Xử lý các thao tác liên quan đến Google. |
...
- Mỗi dòng chứa thông tin một profile theo cấu trúc:
  ```plaintext
  [tên_profile]|[email]|[mật_khẩu_email (option)]|[proxy (tuỳ chọn)]
  ```
  ...
  Trong đó:
    ...
    - `email`: Địa chỉ Gmail.
    - `mật_khẩu_email (option)`: Mật khẩu đăng nhập Gmail. Nếu không có mật khẩu, hãy chọn chế độ "1. Set up" để thực hiện đăng nhập thủ công.
...
- Ví dụ:
  ```plaintext
  profile1|example@gmail.com|12345678|38.154.227.167:2534             // Proxy (không yêu cầu mật khẩu)
  profile2|example@gmail.com|12345678|user:pass@138.154.227.167:2534  // Proxy (yêu cầu mật khẩu)
  profile3|example@gmail.com|12345678                                 // Không dùng proxy
  profile4|example@gmail.com                                          // Không có mật khẩu, đăng nhập thủ công
  ```
